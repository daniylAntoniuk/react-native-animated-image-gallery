...
const ExpandingDot = ({ scrollX, data, dotStyle, containerStyle, inActiveDotOpacity, inActiveDotColor, expandingDotWidth, activeDotColor, onL }) => {
    ...
    return (React.createElement(View, { pointerEvents: 'none', style: [styles.containerStyle, containerStyle] }, data.map((_, index) => {
        const inputRange = [
            (index - 1) * width,
            index * width,
            (index + 1) * width,
        ...
        return (React.createElement(Animated.View, { onLayout: (e) => { onL(e, index); }, key: `dot-${index}`, style: [
                styles.dotStyle,
                dotStyle,
                { width: expand },
                { opacity },
