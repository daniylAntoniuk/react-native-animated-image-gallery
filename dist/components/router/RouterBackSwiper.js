...
export function RouterBackSwiper(props) {
    const { nextIndex, prevIndex, width, animValue, onBackSwipe } = props;
    const navBarHeight = useNavBarHeight();
    const [onGestureEvent] = useState(() => Animated.event([{ nativeEvent: { translationX: animValue } }], {
    ...
    return (React.createElement(PanGestureHandler, { minDist: 5, onGestureEvent: onGestureEvent, onHandlerStateChange: onStateChange },
        React.createElement(Animated.View, { style: [styles.container, { top: navBarHeight }], collapsable: false })));
}
